    ...
    <Dialog.Root open={isOpen} onOpenChange={onChange} defaultOpen={isOpen}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed drop-shadow-md border border-neutral-700 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90vw] md:max-w-[450px] -translate-x-[50%] -translate-y-[50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none">
          ...
          <Dialog.Description className="text-center leading-normal text-sm mb-5">
            {description}
          </Dialog.Description>
          <div>
            {children}
          </div>
          <Dialog.Close className="absolute text-neutral-400 hover:text-white top-[10px] right-[10px] inline-flex h-[25px] w-[25px] appearance-none items-center justify-center rounded-full focus:outline-none transition">
            <IoMdClose />
          </Dialog.Close>
        </Dialog.Content>
      </Dialog.Portal>
